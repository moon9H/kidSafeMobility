...
alarm_path = 'C:/Users/elev0/OneDrive/바탕 화면/수업/종합설계프로젝트1/head-pose-estimation/alarm.wav'
model_file = 'C:/Users/elev0/OneDrive/바탕 화면/수업/종합설계프로젝트1/head-pose-estimation/assets/face_detector.onnx'
face_detector = FaceDetector(model_file)
...
def calculate_angles(bbox):
    """Calculate the angles (pitch, yaw) of the head based on the face bounding box center."""
    ...
    dx = x_center - screen_center_x
    dy = y_center - screen_center_y
    
    yaw = math.degrees(math.atan2(dx, dy))  # 좌우
    pitch = math.degrees(math.atan2(dy, dx))  # 위아래
    
    return yaw, pitch

alarm_triggered = False
start_time = None  
threshold_angle_x = 30
threshold_angle_y = 90
duration = 1.5

video_path = 'C:/Users/elev0/OneDrive/바탕 화면/수업/종합설계프로젝트1/test3.mp4'
output_path = 'C:/Users/elev0/OneDrive/바탕 화면/수업/종합설계프로젝트1/result3.avi'# 비디오 파일 경로
cap = cv2.VideoCapture(video_path)  # 비디오 파일로 대체

# 출력 비디오 초기화
fourcc = cv2.VideoWriter_fourcc(*'XVID')  # 코덱 설정 (예: XVID, MP4V 등)
fps = int(cap.get(cv2.CAP_PROP_FPS))  # 원본 비디오의 프레임 속도
...
while True:
    ret, frame = cap.read()
    if not ret:
        print("비디오 종료 또는 파일 읽기 실패")
        break
    
    detections, _ = face_detector.detect(frame, threshold=0.5)
    
    for det in detections:
        bbox = det[0:4].astype(np.int32)
        yaw, pitch = calculate_angles(bbox)
        cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 2)
        
        label = f"Yaw: {yaw:.2f}, Pitch: {pitch:.2f}"
        cv2.putText(frame, label, (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        
        # if yaw <= threshold_angle_x or pitch >= threshold_angle_y:
        ...
        if yaw >= threshold_angle_x or pitch >= threshold_angle_y:
            ...
            # 화면에 시간 표시
            cv2.putText(frame, f"Time: {elapsed_time:.1f}s", (50, 50), 
            ...
            start_time = None  # 조건이 만족되지 않으면 초기화
            if alarm_triggered:
                alarm_triggered = False
                pygame.mixer.music.stop()            

    cv2.imshow('Face Detection with Angles', frame)
    out.write(frame)
